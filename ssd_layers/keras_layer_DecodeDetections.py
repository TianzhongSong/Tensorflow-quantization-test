...
class DecodeDetections(Layer):
    ...
    def __init__(self,
        ...
        self.dtype = tf.float32 if dtype == 'float32' else tf.float16

        # We need these members for TensorFlow.
        self.tf_confidence_thresh = tf.constant(self.confidence_thresh, dtype=self.dtype, name='confidence_thresh')
        self.tf_iou_threshold = tf.constant(self.iou_threshold, dtype=self.dtype, name='iou_threshold')
        self.tf_top_k = tf.constant(self.top_k, name='top_k')
        self.tf_normalize_coords = tf.constant(self.normalize_coords, name='normalize_coords')
        self.tf_img_height = tf.constant(self.img_height, dtype=self.dtype, name='img_height')
        self.tf_img_width = tf.constant(self.img_width, dtype=self.dtype, name='img_width')
    ...
    def call(self, y_pred, mask=None):
        ...
        def filter_predictions(batch_item):
            ...
            def filter_single_class(index):
                ...
                # revised
                class_id = tf.fill(dims=tf.shape(confidences), value=tf.cast(index, dtype=self.dtype))
                ...
                def no_confident_predictions():
                    return tf.constant(value=0.0, shape=(1,6), dtype=self.dtype)
