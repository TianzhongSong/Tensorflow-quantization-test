...
def _depthwise_conv_block(x, weights, strides=1, block_id=0):
    ...
    w, b = get_weights(weights, conv_name, bias_name)
    x = conv_2d(x, w, b, strides=1, padding='SAME')
    mean, std, beta, gamma = get_bn_param(weights, bn_mean, bn_var, bn_beta, bn_gamma)
    x = batch_norm(x, mean, std, beta, gamma)
...
def MobileNet(img_input, weights, alpha):
    ...
    # init convolution
    w, b = get_weights(weights, 'conv1/kernel:0', 'conv1/bias:0')
    x = conv_2d(x, w, b, strides=2, padding='SAME')
    ...
    w, b = get_weights(weights, 'conv_preds/kernel:0', 'conv_preds/bias:0')
    x = conv_2d(x, w, b, strides=1, padding='SAME')
